function [posdebhfo, posfinhfo, matt]= firrms(x, Fsample,beta,cycles)
...
born_sign_filt = [-max(abs(y)) max(abs(y))]*1.1;
...
for ii=1:length(lo)-1
    if((lo(ii+1)-lo(ii))>=2)
       ...
       if(nombre>=2*cycles)     %seuillage par nombre des pics
           nombrepic(pa)=nombre;
       pa=pa+1;
        vraihfo(inc)= (lo(ii)-u)%vrai hfo.
        posdebhfo(inc1)=u
        posfinhfo(inc2)=lo(ii)
...
matt=zeros(1, n)
for jj=1:1:length(vraihfo)
    matt(posdebhfo(jj):posfinhfo(jj))=1
end
subplot(7,1,7)
        GUI.p_signdetection=imagesc(1:1001,1,matt);
        set(gca,'ydir', 'normal')
        colormap(copper)
        set(gca,'ydir', 'normal')
        title('detected HFOs');
