function [posdebhfo, posfinhfo, matt,matt1,matt2, sig1] = hht(xyt,ni,ns,Fsample,cycles,rmsth)
y=xyt;
Fs=Fsample;   
Ts=1/Fs;      % Sampling period.
imf=emd(y); %  EMD decomposition.
L=size(imf,1);  % size of IMFs.
S=[]; % Matrix inialisation.
...
for jj=1:1:length(aaa)
    matt(aaa(jj):bbb(jj))=1;
end
matt1=zeros(1,length(x));
matt2=zeros(1,length(x));
 for kk=1:length(TAB21)
     if (((max(TAB21{kk}))>250) && ((min(TAB21{kk}))<250))
         ab=min(TAB12{kk});
         ba=max(TAB12{kk});
         matt1(ab:ba)=1;
         matt2(ab:ba)=1;
